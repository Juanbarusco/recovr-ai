...
export default async function handler(req,res){
  try{
    if(!sql)return json(res,503,{error:'DATABASE_URL não configurada.'});
    const session=await requireSession(req);const orgId=session.org.id;
    if(req.method==='GET')return json(res,200,{...(await snapshot(orgId)),workspace:session.org,role:session.role,ai:{configured:aiConfigured()}});
    if(req.method!=='POST')return json(res,405,{error:'Método não permitido.'});
    validateSameOrigin(req);const{action,payload={}}=req.body||{};
    if(action==='health')return json(res,200,{ok:true,database:true,authenticated:true,organization:session.org.name,aiConfigured:aiConfigured()});
    if(action==='runAnalysis'){const created=await runDetectors(orgId);return json(res,200,{created,state:await snapshot(orgId)})}
    if(action==='addContract'){
      const cid=await customer(orgId,payload.client),value=num(payload.value),end=payload.end;if(value<0||!end)throw Object.assign(new Error('Revise valor e vigência do contrato.'),{status:400});
    ...
    if(action==='addReceivable'){
      const cid=await customer(orgId,payload.client),amount=num(payload.amount);if(amount<=0||!payload.due)throw Object.assign(new Error('Revise o valor e o vencimento.'),{status:400});
    ...
    if(action==='recover')return json(res,200,{ok:true,...await registerRecovery(orgId,payload.id,payload.amount)});
    if(action==='analyzeScope')return json(res,200,{analysis:await analyzeScope(orgId,payload)});
    if(action==='createScopeFinding'){
      const c=(await sql`select customer_id,extracted_text from contracts where id=${payload.contract_id} and organization_id=${orgId}`)[0];if(!c)throw Object.assign(new Error('Contrato não encontrado.'),{status:404});
      ...
      const fingerprint=crypto.createHash('sha256').update(`${payload.contract_id}|${payload.source}|${activity.toLowerCase()}`).digest('hex');if(await hasFinding(orgId,'scope',{fingerprint}))return json(res,409,{error:'Essa atividade já possui oportunidade aberta.'});
    ...
    if(action==='analyzeContract')return json(res,200,{analysis:await analyzeContract(payload)});
    if(action==='workspace')return json(res,200,{workspace:session.org,role:session.role,ai:{configured:aiConfigured()}});
    return json(res,400,{error:'Ação desconhecida.'});
  }catch(e){console.error('app-v09',e?.message);return json(res,e?.status||500,{error:e?.message||'Erro interno.',code:e?.code||undefined})}
}
